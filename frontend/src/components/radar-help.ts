...
const fa: HelpMap = {
  ...
  minimumScore: "حداقل امتیاز نهایی Radar برای ارسال هشدار است. امتیاز اصلی فقط از فاکتورهای اولویت‌دار شامل حجم 24h، تعداد معاملات، خریداران یکتای DEX، تغییر قیمت، بازه مارکت‌کپ، فشار خرید، نقدینگی/حجم DEX و Short Squeeze ساخته می‌شود. داده‌های دیگر فقط اطلاعات تکمیلی هستند. این گزینه ۰ ندارد و همیشه فعال است.",
...
const en: HelpMap = {
  ...
  minimumScore: "Minimum final Radar score required for an alert. The primary score uses only the prioritised factors: 24h turnover, trade count, unique DEX buyers, price change, market-cap tier, buy pressure, DEX liquidity/volume and short-squeeze data. Other intelligence is supplementary only. This setting cannot be 0 and is always active.",
